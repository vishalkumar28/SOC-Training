...
graph TD
    ...
    Validation -->|Is this a real security event?| Triage[TRIAGE]
    Triage -->|What is the severity? What is the priority?| Context[CONTEXT GATHERING]
    Context -->|Gather additional information| Investigation[INVESTIGATION]
    Investigation -->|Dig deeper into the event| Severity[SEVERITY ASSESSMENT]
    Severity -->|Assign final severity| Escalation[ESCALATION]
    Escalation -->|Escalate if needed| Documentation[DOCUMENTATION]
```
Stage 1: Alert
