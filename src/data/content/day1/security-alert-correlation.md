...
```mermaid
graph TD
    A["1. 4625: jsmith - Failed login (203.0.113.45)"] -->|50 failures over 10 minutes| B["2. 4624: jsmith - Successful login (203.0.113.45)"]
    B -->|Attacker now has access| C["3. 4688: powershell.exe launched by jsmith"]
    C -->|Attacker running commands| D["4. 4688: payload.exe downloaded and executed"]
    D -->|Malware installed| E["5. 5156: Network connection to 203.0.113.100"]
    E -->|Command and control| F["6. 4663: Sensitive file accessed"]
    F -->|Data theft| G([End])
```
