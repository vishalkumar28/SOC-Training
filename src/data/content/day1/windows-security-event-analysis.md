...
Logon Type: 10
Time: 2024-11-15 03:14:22 UTC
Why suspicious? Administrator login via RDP from an external IP at 3 AM.
...
Logon Type: 2
Time: 2024-11-15 09:05:00 UTC
Why benign? Normal interactive login from internal network during business hours.

Investigation Questions:

Is the source IP expected?

Is the time of day normal for this user?

Is the logon type appropriate for the scenario?
...
Why It Matters:
Failed logins are normal (users forget passwords), but a high volume of failures from the same IP or targeting the same account is suspicious.

Important Fields:

Field	What It Tells You
Account Name	Which account was targeted
Source Network Address	IP address of the attacker
...
Multiple failures for the same account (brute force)

Failures for many different accounts from the same IP (password spraying)

Failures followed by success (compromised credentials)

Suspicious Pattern:

text
Event ID: 4625 (x15 in 5 minutes)
Account Name: jsmith
Source Network Address: 203.0.113.45
Failure Reason: Unknown user name or bad password
Time: 2024-11-15 09:23:00-09:28:00
Why suspicious? 15 failures in 5 minutes is brute-force behavior.

Event ID 4688: Process Creation
Name: A new process has been created

Why It Matters:
This event tells you what programs are running on a system. Attackers use process execution to run malware, tools, and scripts.

Important Fields:

Field	What It Tells You
New Process Name	Full path of the executable
Creator Process Name	What program started this process
Command Line	The full command used (if enabled)
Account Name	Which account ran the process
Important Note: Command line logging must be enabled separately

SOC Use Case:
Monitor for:

Unknown executables in suspicious locations (Temp, Downloads)

PowerShell with encoded commands

Use of system tools (wmic, netsh) for malicious purposes

Processes launched from Office applications
